...
import AddProduct from '../AddProductToWarehouse';
...
const Warehouseisting = ({
  ...
  const[isAddProduct,onSetIsAddProduct] = useState(false);
  ...
  const handelAddProduct = ()=>{
    onSetIsAddProduct(true);
  }

  const handleAddProductClose = ()=>{
    onSetIsAddProduct(false)
  }
  const onOpenAddProduct = (id) => {
    setIdWarehouse(id)
    handelAddProduct();
  };
  ...
  const onChangeStarred = (status, contact) => {
    ...
    putDataApi(`${baseURL}/api/contactApp/update/starred`, infoViewActionsContext, {
  ...
  const onDeleteSelectedContacts = () => {
    postDataApi(`${baseURL}/api/warehouse/delete/${toDeleteWarehouse}`, infoViewActionsContext)
      .then(() => {
        reCallAPI();
